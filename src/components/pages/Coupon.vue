...
export default {
  ...
  methods: {
    getCoupons(page = 1) {
      const api = `${process.env.APIPATH}/api/${process.env.CUMSTOMPATH}/admin/coupons?page=${page}`;
      const vm = this;
    ...
    delCoupon() {
      const vm = this;
      const api = `${process.env.APIPATH}/api/${process.env.CUMSTOMPATH}/admin/coupons/${vm.coupons.id}`;
